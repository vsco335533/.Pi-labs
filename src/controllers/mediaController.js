...
export const uploadMedia = async (req, res) => {
  try {
    ...
    // Only use category if provided in body
    const { image_category_id, description } = req.body;

    const result = await query(
      `
      INSERT INTO media
        (title, type, url, thumbnail_url, file_size, uploaded_by, post_id, status, public_id, image_category_id, description)
      VALUES
...
export const updateMedia = async (req, res) => {
  try {
    const { title, description, image_category_id } = req.body;
    const { id } = req.params;

    // Optional: Check if user is admin (handled by middleware usually, but good to be safe)
    if (req.user.role !== "super_admin") {
      return res.status(403).json({ error: "Not allowed" });
    }

    // Check if media exists
    const check = await query("SELECT id FROM media WHERE id = $1", [id]);
    ...
    const result = await query(
      ...
      SET title = COALESCE($1, title),
          ...
          image_category_id = $3, -- Allow setting to null or new value
          updated_at = NOW()
      WHERE id = $4
      RETURNING *
      `,
      [title, description, image_category_id || null, id]
    );
